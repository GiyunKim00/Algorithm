...
void recursive(vector<vector<int>> &arr, pair<int, int> start, int size) {
    ...
    if (check) {
        ...
        recursive(arr, {start.first, start.second + 2 * new_size}, new_size);
        ...
        recursive(arr, {start.first + new_size, start.second + 2 * new_size}, new_size);
        recursive(arr, {start.first + 2 * new_size, start.second}, new_size);
        recursive(arr, {start.first + 2 * new_size, start.second + new_size}, new_size);
        recursive(arr, {start.first + 2 * new_size, start.second + 2 * new_size}, new_size);
    } else ans[num]++;
}
